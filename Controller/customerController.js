...
exports.getCustomerDashboard = async (req,res)=>{
    try {
        ...
        let restaurantQuery = {};
        if(locationFilter){
            restaurantQuery.location = { $regex: new RegExp(locationFilter.trim(), 'i') };
        }
        ...
        prev_order.forEach(order => {
            ...
            if(order.name){
                restaurantCountMap[order.name] = (restaurantCountMap[order.name] || 0) + 1;
            }
        });
...
exports.getFeedBack = async (req, res) => {
    try {
        ...
        const recentOrders = await Order.find({ customerName: username }).sort({ _id: -1 }).limit(1);
...
exports.postOrderAndReservation = async (req, res) => {
    try {
        ...
        const user = req.user;
        if(user){
            const person = await require('../Model/customer_model').findOne({ email: user.email });
